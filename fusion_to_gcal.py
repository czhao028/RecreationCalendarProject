...
def get_current_events(feed_url_or_path, files):
    """Retrieves data from Fusion per calendar and returns a dictionary {str: list} of Pool Lane to Calendar Events

    Returns the parsed list of events per pool or None if an error occurs.
    """
    return None
...
def create_id(uid, begintime, endtime, prefix=''):
    ...
    return prefix + re.sub('[^{}]'.format(allowed_chars), '', uid.lower()) + str(
        arrow.get(begintime).int_timestamp) + str(arrow.get(endtime).int_timestamp)
...
if __name__ == '__main__':
    ...
    for feed in config.get('ICAL_FEEDS'):
        ...
        gcal_events = get_gcal_events(calendar_id=feed['destination'], service=service, from_time=(
                    today - timedelta(days=config.get('PAST_DAYS_TO_SYNC', 0))).isoformat())
        ...
        for gcal_event in gcal_events:
            ...
            if eid not in ical_events:
                ...
                try:
                    ...
                    logger.info(u'> Deleting event "{}" from Google Calendar...'.format(
                ...
                locs_differ = gcal_has_location != ical_has_location and gcal_event.get(
        ...
        for ical_id, ical_event in ical_events.items():
            if ical_id not in gcal_event_ids:
                ...
                logger.info(
                    'Adding iCal event called "{}", starting {}'.format(ical_event.summary, gcal_event['start']))
                ...
                try:
                    ...
                    time.sleep(config['API_SLEEP_TIME'])
                    try:
                        service.events().update(calendarId=feed['destination'], eventId=gcal_event['id'],
                                                body=gcal_event).execute()
                    except Exception as ex:
